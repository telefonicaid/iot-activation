...
<p align="center">
      <img title="Telefonica_SIM" src="pictures/Telefonica_SIM.jpg"
...
Another alternative for development on Arduino boards without the need for downloads is its new web editor 
https://create.arduino.cc/editor allowing the same development as on the desktop editor.

to use this online tool you only need to register on the Arduino website and create a user account.

![pic](pictures/Arduino/Arduino_WEB_login.png)

This editor presents a friendly interface but with the same functionalities. As you can see in the following screenshot 
...
Because it will not be necessary to manage the libraries and the plates

![pic](pictures/Arduino/Arduino_WEB_editor_board.png)

In this case, you should look for the model of your board, MKR NB 1500
...
Discontinuous reception (DRX) is a way that mobile communication are used to save the battery of the mobile device.
...
When the PSM is activated, the device will perform the IDLE stage and then turn off the connection. 
But as we mentioned earlier, every time we connect to the network, Attachment will happen again. 
Well! the advantage of this way is that when you configured it, the device warns the network of the time it remains deactivated, 
...
![pic](pictures/schematics/LTE_table_value_ERX.png)
...
the times of connection and rest copying the codes that there are in the table. 
```AT
** Activate PSM: 1h connected / 5m resting"
...
with the last 5 digits with (SOON)
