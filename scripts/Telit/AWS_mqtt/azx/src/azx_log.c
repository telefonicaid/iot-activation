...
#define LOG_FILE_PREFIX(tag) \
    case AZX_LOG_LEVEL_##tag: \
      offset = snprintf(log_buffer, LOG_BUFFER_SIZE, \
          prefix_fmt_file, get_date_time(), #tag, \
          (now / 1000.0), \
          get_file_title(file), line \
      ); \
    break
...
} logFile = { .fd = 0, .name = { 0 }, .max_size_kb = 0, .min_level = AZX_LOG_LEVEL_CRITICAL };
...
static const CHAR* prefix_fmt_file = "%s [%-5s] %6.2f %s:%d - ";
...
static const char* get_date_time()
{
  ...
  m2mb_rtc_close( fd );
  snprintf(dateTime, sizeof(dateTime), "%u-%02u-%02u %02u:%02u:%02u",
    ...
    const char* function, const char* file, int line, const CHAR *fmt, ... )
{
  ...
  if(level >= azx_log_getLevel())
  {
    ...
    {
      if(check_file_size(logFile.name, logFile.max_size_kb))
      {
        switch(level)
        {
        ...
        vsnprintf(log_buffer + offset, LOG_BUFFER_SIZE - offset, fmt, arg);
        va_end(arg);
        m2mb_fs_fputs(log_buffer, logFile.fd);
      }
      else
      {
...
BOOLEAN azx_log_send_to_file(const CHAR* filename, AZX_LOG_LEVEL_E min_level, UINT32 max_size_kb)
{
  if(!filename)
  {
    return FALSE;
  ...
  if(!check_file_size(filename, max_size_kb))
  {
    return FALSE;
  }
  ...
  logFile.fd = m2mb_fs_fopen(filename, "a");
