...
/* SSL */
#define CACERTFILE        "/mod/ssl_certs/awsCACert.crt"   /* Root CA file path in module filesystem (if needed) */
#define CLIENTCERTFILE    "/mod/ssl_certs/xxxxxxxxxx-certificate.pem.crt"   /* Client certificate file path in module filesystem (if needed) */
#define CLIENTKEYFILE     "/mod/ssl_certs/xxxxxxxxxx-private.pem.key"   /* Client private key file path in module filesystem (if needed) */
...
M2MB_SSL_CIPHER_SUITE_E M2MB_SSL_DEFAULT_CIPHER_SUITES[]={
                         M2MB_TLS_RSA_WITH_AES_128_CBC_SHA,
                         M2MB_TLS_RSA_WITH_AES_256_CBC_SHA,
                         M2MB_TLS_ECDH_ECDSA_WITH_AES_128_GCM_SHA256, //missing TLS_RSA_WITH_3DES_EDE_CBC_SHA and TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA
...
//  M2MB_SSL_CA_INFO_T ca_Info[1];
  ...
  *p_hSSLCtx =  m2mb_ssl_create_ctxt();
  ...
  SSLConfig.CipherSuites = &M2MB_SSL_DEFAULT_CIPHER_SUITES[0];
                  ...
                  void *myUserdata )
{
  ...
  switch( net_event )
  {
    case M2MB_NET_GET_REG_STATUS_INFO_RESP:
      ...
      {
        AZX_LOG_DEBUG( "Module is registered. AcT: %d\r\n", stat_info->rat );
...
INT32 MQTT_Task( INT32 type, INT32 param1, INT32 param2 )
{
  ...
  do
  {
    ...
    {
#if 1
      if(0 != m2mb_ssl_config( sslConfigHndl, M2MB_SSL_NAME_SNI, (void*)MQTT_BROKER_ADDRESS ))
      {
    ...
    while(1)
    {
      ...
      sprintf( msg, "{\"state\": {\"reported\" : { \"temp\" : %d}}}", msg_value );
      AZX_LOG_DEBUG( "PUBLISHING <%s> to topic %s\r\n", msg, topic );
      result = m2mb_mqtt_publish( mqttHandle, M2MB_MQTT_QOS_0, retain, msg_value, topic, msg, strlen( msg ) );
